...
const UserReport = () => {
  const { openUserReport } = useSelector((state) => state.user);
  ...
  const Backdrop = () => {
    return (
      <div
        ...
        onClick={() => dispatch(setOpenUserReport({ openUserReport: false }))}
      />
    );
  };
    ...
    return (
      ...
      >
        <header className="flex justify-between  p-6">
          <h1 className="text-2xl">Users Reports</h1>
          <FontAwesomeIcon
            icon={faXmark}
            className={`${
            ...
            onClick={() => {
              dispatch(setOpenUserReport({ openUserReport: false }));
            }}
          />
        </header>
      ...
      {openUserReport && (
        <>
          {ReactDOM.createPortal(
            <Backdrop />,
            document.getElementById("backdrop-user-report-root")
          )}
          {ReactDOM.createPortal(
            <Overlay />,
            document.getElementById("overlay-user-report-root")
          )}
        </>
      )}
    </>
